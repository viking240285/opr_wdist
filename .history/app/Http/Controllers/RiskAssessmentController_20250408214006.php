...
class RiskAssessmentController extends Controller
{
    ...
    public function create(RiskMap $riskMap): View
    {
        ...
        // TODO: Get Measures for select dropdowns if implementing measure selection
        // $measures = Measure::orderBy('description')->pluck('description', 'id')->toArray();
        return view('risk-assessments.create', compact('riskMap', 'hazards'));
    }
    ...
    public function edit(RiskAssessment $assessment): View // Shallow binding
    {
        ...
        // Get hazards, allowing the current one to be selected
        $hazards = $this->getHazardsForSelect($riskMap, $assessment);
        // Ensure the current hazard is available if it was somehow excluded
        if (!isset($hazards[$assessment->hazard_id])) {
            ...
            if ($currentHazard) {
                ...
                // Consider sorting $hazards array again by key (code) if needed
            }
        }

        // TODO: Get Measures
        return view('risk-assessments.edit', compact('riskMap', 'assessment', 'hazards'));
    }
