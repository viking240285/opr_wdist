...
class WorkplacePolicy
{
    ...
    /**
     * Определяет, может ли пользователь создавать рабочие места в конкретной организации.
     * Примечание: Администратор уже получил доступ через метод before.
     *
     * @param  \App\Models\User  $user
     * @param  \App\Models\Organization $organization Контекст организации, в которой создается РМ
     * @return bool
     */
    public function create(User $user, \App\Models\Organization $organization): bool
    {
        // Метод before уже разрешил доступ администратору.
        // Проверяем, может ли Менеджер или Руководитель создавать РМ в *своей* организации.
        if ($user->isManager() || $user->isDepartmentHead()) {
            // Дополнительно проверяем, что пользователь принадлежит к той организации,
            // в которой пытается создать рабочее место.
