...
@php
    ...
    } else if ($multiple && !is_array($initialSelected)) {
...
<div x-data="selectComponent({{ json_encode($alpineInitData) }})"
    ...
    >
        {{-- Пустая опция для placeholder в одиночном выборе --}}
        <template x-if="!multiple && placeholder">
             <option value=""></option>
        </template>
    ...
    >
        ...
        <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
            <svg class="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path fill-rule="evenodd" d="M10 3a.75.75 0 01.53.22l3.75 3.75a.75.75 0 01-1.06 1.06L10 5.06 6.78 8.28a.75.75 0 01-1.06-1.06l3.75-3.75A.75.75 0 0110 3zm10 14a.75.75 0 01-.53.22l-3.75-3.75a.75.75 0 111.06-1.06L10 14.94l3.22-3.22a.75.75 0 111.06 1.06l-3.75 3.75A.75.75 0 0110 17z" clip-rule="evenodd" />
                 {{-- Исправленная иконка из breeze: chevron-up-down --}}
    ...
    >
        ...
        <div x-show="searchable" class="{{ $searchClasses }}"> {{-- Убрали li, добавили стили обертке --}}
            <input
                ...
                class="w-full border-0 focus:ring-0 focus:outline-none bg-transparent dark:text-gray-300"
                @click.stop
                x-ref="searchInput"
            />
        </div>
            ...
            <template x-for="(option, index) in filteredOptions" :key="option.value">
                ...
                >
                    <span class="block truncate" :class="{ 'font-semibold': isSelected(option.value) }" x-text="option.label"></span>
                     ...
                     <span x-show="isSelected(option.value)" class="absolute inset-y-0 right-0 flex items-center pr-4 text-indigo-600 dark:text-white" :class="{ 'text-white': activeIndex === index }">
...
<script>
    function selectComponent(initData) {
        return {
            ...
            init() {
                ...
                if (this.multiple) {
                    this.selectedValues = Array.isArray(this.initialSelected)
                        ? this.initialSelected.filter(val => this.options.some(opt => opt.value == val)) // Оставляем только существующие
                        : [];
                } else {
                     this.selectedValue = this.options.some(opt => opt.value == this.initialSelected)
                        ? this.initialSelected
                        : null; // Устанавливаем null, если значение не найдено
                }
            ...
            isSelected(value) {
                if (this.multiple) {
                    return this.selectedValues.includes(String(value)); // Сравнение строк
                }
                return this.selectedValue === String(value); // Сравнение строк
            },
            ...
            toggleOption(value) {
                 ...
                 if (this.multiple) {
                    ...
                    // Чтобы разрешить снятие выбора, нужно добавить условие:
                    // this.selectedValue = this.isSelected(stringValue) ? null : stringValue;
                    this.selectedValue = stringValue;
                    this.closeDropdown(); // Закрываем после выбора
                }
             ...
             updateHiddenSelect() {
                ...
                if (this.multiple) {
                    this.selectedValues.forEach(val => {
                        const option = select.querySelector(`option[value="${CSS.escape(val)}"]`); // Используем CSS.escape
                        if (option) option.selected = true;
                    });
                } else if (this.selectedValue !== null) {
                    const option = select.querySelector(`option[value="${CSS.escape(this.selectedValue)}"]`); // Используем CSS.escape
                    if (option) option.selected = true;
                }
