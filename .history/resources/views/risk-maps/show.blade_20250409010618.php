                                    ...
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            @can('update', $assessment)
                                                <a href="{{ route('assessments.edit', $assessment) }}" class="text-yellow-600 dark:text-yellow-400 hover:text-yellow-900 dark:hover:text-yellow-300 mr-3">{{ __('Редакт.') }}</a>
                                            @endcan
                                            {{-- Кнопка "Удалить" оценку --}}
                                            @can('delete', $assessment)
                                                <form action="{{ route('assessments.destroy', $assessment) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('Вы уверены, что хотите удалить эту оценку риска?') }}');">
                                                    @csrf
                                                    @method('DELETE')
