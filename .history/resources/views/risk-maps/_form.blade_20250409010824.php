...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div class="md:col-span-2 border-t pt-4 mt-4 border-gray-200 dark:border-gray-700"
         x-data="participantsManager({{ json_encode(old('participants', $riskMap->participants ?? [])) }})"
         x-init="init()">

        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">{{ __('Участники оценки (необязательно)') }}</h3>

        {{-- Скрытое поле для отправки JSON --}}
        <input type="hidden" name="participants" :value="jsonParticipants">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    <template x-for="(participant, index) in participants" :key="index">
                        <tr>
                            {{-- Убираем роль --}}
                            <td class="px-2 py-1 whitespace-nowrap">
                                <x-text-input x-model="participant.name" class="mt-1 block w-full text-sm" type="text" placeholder="{{ __('ФИО') }}" />
                            </td>
                            <td class="px-2 py-1 whitespace-nowrap">
                                <x-text-input x-model="participant.position" class="mt-1 block w-full text-sm" type="text" placeholder="{{ __('Должность') }}" />
                            </td>
                            <td class="px-2 py-1 whitespace-nowrap text-right text-sm font-medium">
                                <button type="button" @click="removeParticipant(index)" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" title="{{ __('Удалить') }}">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                </button>
                            </td>
                        </tr>
                     ...
                     <template x-if="participants.length === 0">
                         <tr>
                             <td colspan="3" class="px-2 py-4 text-center text-sm text-gray-500 dark:text-gray-400">
                                 {{ __('Участники не добавлены.') }}
                             </td>
                         </tr>
                     </template>
                </tbody>
        ...
        {{-- Кнопка Добавить --}}
        <div class="flex justify-end">
            <x-secondary-button type="button" @click="addParticipant">
                {{ __('Добавить участника') }}
            </x-secondary-button>
        </div>
         {{-- Выводим ошибку валидации для participants (если нужна) --}}
...
<script>
    ...
    function participantsManager(initialParticipants) {
        // Убедимся, что initialParticipants это массив
        let initialData = [];
        if (typeof initialParticipants === 'string' && initialParticipants.length > 0) {
            try {
                initialData = JSON.parse(initialParticipants);
                if (!Array.isArray(initialData)) initialData = [];
            } catch (e) {
                initialData = []; // Если JSON невалидный, начинаем с пустого списка
            }
        } else if (Array.isArray(initialParticipants)) {
             initialData = initialParticipants;
        }

        return {
            participants: initialData,
            jsonParticipants: '', // Сюда будем писать JSON для отправки

            init() {
                this.updateJsonField(); // Обновляем скрытое поле при инициализации
                 // Следим за изменениями в массиве participants и обновляем JSON
            ...
            updateJsonField() {
                 // Удаляем пустые строки перед сохранением в JSON
                 const validParticipants = this.participants.filter(p => p.name || p.position);
                 this.jsonParticipants = JSON.stringify(validParticipants);
            }
        }
    }
</script>
